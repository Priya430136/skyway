...
import {
  Search, Luggage, PlaneTakeoff, TicketCheck, Award, Wallet,
  BarChart3, Sparkles, LifeBuoy, ChevronLeft, Menu, X, LayoutDashboard,
  MapPin,
} from "lucide-react";
...
const NAV = [
  { section: "Travel", items: [
    { to: "/app/dashboard", label: "User Dashboard", icon: LayoutDashboard, matchPrefix: "/app/dashboard" },
    { to: "/app", label: "Book", icon: Search, exact: true },
    { to: "/app/my-trips", label: "My Trips", icon: Luggage },
    { to: "/app/flight-status/SW128", label: "Flight Status", icon: PlaneTakeoff, matchPrefix: "/app/flight-status" },
    { to: "/app/terminal-map", label: "Terminal Map", icon: MapPin, matchPrefix: "/app/terminal-map" },
    { to: "/app/baggage", label: "Baggage Tracker", icon: Luggage, matchPrefix: "/app/baggage" },
    { to: "/app/check-in/QX7A2K", label: "Check-in", icon: TicketCheck, matchPrefix: "/app/check-in" },
  ]},
  { section: "Account", items: [
    { to: "/app/loyalty", label: "Miles & Rewards", icon: Award },
    { to: "/app/wallet", label: "Wallet", icon: Wallet },
    { to: "/app/analytics", label: "Travel Analytics", icon: BarChart3 },
  ]},
  { section: "Support", items: [
    { to: "/app/assistant", label: "AI Assistant", icon: Sparkles },
    { to: "/app/help", label: "Help Center", icon: LifeBuoy, matchPrefix: "/app/help" },
  ]},
];
    ...
    <>
      <div className="flex items-center justify-between px-4 py-4 border-b border-white/10">
        {!collapsed && (
          <Link to="/" className="font-display text-lg tracking-tight">
            SkyWay <span className="text-sky-gold">Travel</span>
          </Link>
        )}
        ...
        {NAV.map((group) => (
          <div key={group.section} className="mb-4">
            ...
            <ul className="space-y-0.5 px-2">
              {group.items.map((it) => {
                const prefix = (it as { matchPrefix?: string }).matchPrefix ?? it.to;
      ...
      {!collapsed && (
        <div className="border-t border-white/10 px-4 py-3 text-[10px] text-white/40">
          <div>SkyWay Passenger · v2.4</div>
          <div className="mt-1 flex items-center gap-1.5"><span className="h-1.5 w-1.5 rounded-full bg-emerald-400 animate-pulse" /> All services online</div>
        </div>
      )}
    </>
  );
