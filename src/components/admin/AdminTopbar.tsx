...
import { notifications as notifs } from "@/lib/admin/mock";
import { AccountMenu } from "@/components/AccountMenu";
import { PerformanceIndicator } from "@/components/PerformanceIndicator";
import { GlobalSearchBar } from "@/components/GlobalSearchBar";

export function AdminTopbar({ crumbs, action }: { crumbs: { label: string; to?: string }[]; action?: { label: string; onClick?: () => void } }) {
  const [openBell, setOpenBell] = useState(false);
  const unread = notifs.filter((n) => n.severity === "high" || n.severity === "critical").length;
    ...
    <header className="sticky top-0 z-20 border-b border-border bg-background/85 backdrop-blur">
      <div className="flex items-center gap-4 px-6 py-3 pl-16 md:pl-6">
        ...
        <div className="ml-auto hidden md:block">
          <GlobalSearchBar placeholder="Search flights (SW101), passengers, tickets, fleet…" />
        </div>
        ...
        <PerformanceIndicator portalScope="admin" />
          ...
          {openBell && (
            <div className="absolute right-0 top-11 z-30 w-96 overflow-hidden rounded-xl border border-border bg-popover shadow-xl">
              <div className="flex items-center justify-between border-b border-border px-4 py-3">
                ...
                <Link to="/admin/notifications" onClick={() => setOpenBell(false)} className="text-xs text-sky-accent hover:underline">View all</Link>
              </div>
        ...
        {/* Account & Role Menu */}
        <AccountMenu currentPortalRole="admin" />
      </div>
    </header>
  );
}
